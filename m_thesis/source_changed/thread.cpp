...
Thread::set_sched_params(L4_sched_param const *p)
{
  ...
  printf("[Thread.cpp:set_sched_params)] Setting the params\n");
}
...
Thread::start_migration()
{
  ...
  if (m->cpu == home_cpu())
    {
	  printf("[Thread.cpp:Start_migration] Set the scheduling params \n");
...
Thread::migrate(Migration *info)
{
  ...
  printf("[Thread.cpp] Migrate Called 1\n");
...
Thread::migrate(Migration *info)
{
	printf("[Thread.cpp] Migrate Called 2\n");
  assert_kdb (cpu_lock.test());
  ...
  LOG_TRACE("Thread migration", "mig", this, Migration_log,
  ...
  );
    {
      ...
      do{
        old = _migration;
        printf("[Thread.cpp:migrate] In do while loop\n");
  ...
  if (current_cpu() == cpu || Config::Max_num_cpus == 1){
	  printf("[Thread.cpp:migrate] current = cpu %d of thread\n", cpu);
	  current()->schedule_if(do_migration());
  }
  else{
	  printf("[Thread.cpp:migrate] the else case %d\n", cpu);
    current()->schedule_if(migrate_xcpu(cpu));
    printf("[Thread.cpp:migrate] thread migrated\n", cpu);
  }
  cpu_lock.clear();
...
Thread::migrate_away(Migration *inf, bool remote)
{
  ...
  Cpu_number target_cpu = inf->cpu;

    {
      ...
      Sched_context *sc = sched_context();
      printf("Setting the priority and scheduler\n");
      sc->set(inf->sp);
      sc->replenish();
      set_sched(sc);

      printf("Setting scheduler done\n");
      Mem::mp_wmb();
...
{
	printf("[Thread:migrate_to] Start\n");
  bool ipi = false;
    {
      ...
      if (target_cpu == current_cpu())
        {
          ...
          return true;
          //return resched | Sched_context::rq.current().deblock(sched(), current()->sched()); //gmc: commented out
        }
      printf("[Thread:migrate_to] Mid1\n");
      if (!_pending_rq.queued())
        {
          if (!q.first())
            ipi = true;
  ...
  bool ipi = false;

    {
      ...
      if (cpu != access_once(&_home_cpu)){
    	  printf("[Thread.cpp:migrate_xcpu] Already migrated %d\n", cpu);
    	  return false;
      }
      ...
      if (!Cpu::online(cpu))
        {
    	  printf("[Thread.cpp:migrate_xcpu] Start migration called %d\n", cpu);
          Migration *inf = start_migration();
          ...
          Cpu_number target_cpu = access_once(&inf->cpu);
          printf("[Thread.cpp:migrate_xcpu] Migrate away called %d\n", cpu);
          migrate_away(inf, true);
          printf("[Thread.cpp:migrate_xcpu] Migrate away return %d\n", cpu);
          g.reset();
          printf("[Thread.cpp:migrate_xcpu] migrate to  called %d\n", target_cpu);
          bool ret =  migrate_to(target_cpu, true);
          printf("[Thread.cpp:migrate_xcpu] migrate returned %d\n", ret);
          return ret;
